    ...
    >
      <div className="m-x-auto h-[calc(100vh-72px)] w-[85vw] flex items-center px-0 text-white lg:px-24 md:px-16">
        <div className="max-md:absolute max-md:top-[3rem]">
          {/* Logo */}
          <h1 className="mb-10 text-[3rem] text-white font-bold md:text-[6rem]">{t('Consult.logo')}</h1>

          {/* 主标题 */}
          <h2 className="mb-8 text-xl leading-tight md:text-[3rem]">{t('Consult.title')}</h2>

          {/* 副标题 */}
          <p className="mb-12 w-[60%] text-xs text-gray-300 md:w-[31vw] md:text-2xl">{t('Consult.subtitle')}</p>

          {/* 申请按钮 */}
          <Button
            type="primary"
            size="large"
            className="mr-10 transition-all duration-300 hover:scale-105"
            style={{
          ...
          <Button
            ...
            onClick={() => window.open('http://preview.sub.admin.metakina.com', '_blank')}
          >
            {t('Consult.ht')}
          </Button>
        </div>
